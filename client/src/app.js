...
function line(cv, series, opts = {}) {
  ...
  if (opts.base != null) { const y = H - (opts.base - lo) / (hi - lo) * H; g.strokeStyle = opts.baseColor || '#3a3a4a'; g.setLineDash([3, 4]); g.beginPath(); g.moveTo(0, y); g.lineTo(W, y); g.stroke(); g.setLineDash([]); }
...
function renderState() {
  ...
  const mon = $('mon'); if (S.coins.length) { line(mon, S.coins.slice(0, 8).map((c) => ({ pts: c.agent.curve.map((p) => [p[0], p[1] / c.agent.seed * 100]), color: c.color, w: 1.6 })), { base: 100, baseColor: '#3a3a4a', glow: true }); $('mon-legend').innerHTML = S.coins.slice(0, 8).map((c) => `<span><i style="background:${c.color}"></i>${c.ticker} ${sgn((c.agent.equity / c.agent.seed - 1) * 100, 1)}%</span>`).join(''); }
  else { const g = mon.getContext('2d'); mon.width = mon.clientWidth; mon.height = mon.clientHeight; g.strokeStyle = '#9d7bff'; g.lineWidth = 2; g.beginPath(); for (let x = 0; x < mon.width; x++) { const y = mon.height / 2 + (x % 90 > 40 && x % 90 < 52 ? Math.sin((x % 90 - 40) / 12 * Math.PI) * 60 : 0); x ? g.lineTo(x, y) : g.moveTo(x, y); } g.stroke(); $('mon-legend').innerHTML = '<span>flatline · no agents yet</span>'; }
...
function renderCoin() {
  ...
  const cv = $('c-canvas'); if (chartMode === 'eq') line(cv, [{ pts: a.curve, color: c.color, w: 2.2 }], { base: a.seed, glow: true }); else line(cv, [{ pts: c.trades.slice().reverse().map((t) => [t.ts, t.px]).concat([[Date.now(), c.price]]), color: '#f1f0f6', w: 2 }]);
