...
const E = (n) => (n == null || !isFinite(n)) ? '—' : (Math.abs(n) >= 100 ? (+n).toFixed(2) : Math.abs(n) >= 1 ? (+n).toFixed(4) : (+n).toFixed(5)) + ' ETH';
...
const px = (p) => (+p).toExponential(2) + ' ETH';
...
function renderState() {
  ...
  $('s-agents').textContent = P.agents; $('s-eq').textContent = E(P.agentsEquity); $('s-bb').textContent = E(P.buybackEth); $('s-vol').textContent = E(P.volume); $('s-launch').textContent = P.launches;
  ...
  $('feed').innerHTML = S.feed.map((e) => `<div class="r"><span>${ago(e.ts)}</span><span>${e.type === 'launch' ? `<b>${e.coin}</b> launched with a ${e.brain} brain and ${E(e.eth)}` : e.type === 'buyback' ? `<b>${e.coin}</b> agent bought back ${E(e.eth)} of its coin` : e.type}</span></div>`).join('') || '<div class="r"><span>—</span><span>quiet ward</span></div>';
}
...
function renderCoin() {
  ...
  $('c-log').innerHTML = (a.log || []).map((l) => `<div class="r"><span>${ago(l.ts)}</span><span>${l.kind === 'open' ? `opened ${l.side} ${l.sym} ${E(l.notional)} @ $${fmt(l.px, 2)}` : l.kind === 'close' ? `closed ${l.side} ${l.sym} <span class="${l.pnl >= 0 ? 'up' : 'dn'}">${sgn(l.pnl, 5)}</span> · ${l.why}` : `<span class="v">bought back</span> ${E(l.eth)} → ${big(l.coins)} ${c.ticker}`}</span></div>`).join('') || '<div class="r"><span>—</span><span>no trades yet</span></div>';
...
function renderAccount() {
  $('d-w').textContent = wallet || 'not connected'; $('d-min').textContent = S ? S.minDeposit : 0.005; $('d-tre').textContent = S ? S.treasury.slice(0, 8) + '…' + S.treasury.slice(-6) : '—';
  $('d-usdg').textContent = A ? E(A.eth) : '—'; $('d-dep').textContent = A ? E(A.deposited) : '—'; $('d-val').textContent = A ? E(A.coins.reduce((a, c) => a + c.value, 0)) : '—';
  ...
  $('d-queue').innerHTML = A && A.queue.length ? A.queue.map((q) => `<div class="row"><span>withdraw ${E(q.amt)} · ${q.id}</span><b>${q.status === 'paid' ? 'paid' + (q.tx ? ' · ' + q.tx.slice(0, 10) + '…' : '') : 'queued'}</b></div>`).join('') : '';
...
$('d-send').onclick = async () => {
  if (needWallet()) return; const amount = +$('d-in').value; const minDep = (S && S.minDeposit) || 20; if (!amount || amount < minDep) return toast('minimum deposit is ' + minDep + ' ETH', true);
  const eth = evm(); if (!eth) return toast('open a wallet to send ETH, or paste a tx hash', true); if (!S || !S.treasury) return toast('treasury not configured', true);
  try { await ensureChain(eth); const value = '0x' + BigInt(Math.round(amount * 1e18)).toString(16);
    const tx = await eth.request({ method: 'eth_sendTransaction', params: [{ from: wallet, to: S.treasury, value }] }); toast('sent · waiting for the receipt…');
